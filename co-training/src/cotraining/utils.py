...
class dino_ENS(torch.nn.Module):
    ...
    def forward(self, x, with_variance=False):
        ...
        if with_variance:
            pred = (torch.sum(predictions, 0) / len(self.mod_list)) / (1 + (0.3*torch.var(predictions, 0)))**(0.5)
            pred_idx = torch.argmax(pred, -1)
            smax = torch.nn.functional.softmax(predictions, -1)
            unc = 1.0 - torch.gather(smax, -1, pred_idx.unsqueeze(-1)).squeeze()
            return pred, unc
        else:
            return (torch.sum(predictions, 0) / len(self.mod_list)) / (1 + (0.3*torch.var(predictions, 0)))**(0.5)
